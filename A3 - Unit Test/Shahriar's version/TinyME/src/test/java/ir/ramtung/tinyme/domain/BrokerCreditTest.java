...
public class BrokerCreditTest {
    ...
    void increase_credit_by_negative_amount_throws_exception() {
        assertThrows(AssertionError.class, () -> broker.increaseCreditBy(-50_000));
    }

    @Test
    void decrease_credit_by_negative_amount_throws_exception() {
        assertThrows(AssertionError.class, () -> broker.decreaseCreditBy(-50_000));
    }

    @Test
    void credit_changes_after_order_execution() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 6, java.time.LocalDateTime.now(), Side.SELL, 350, 15700, 0, 0, 0);
        matcher = new Matcher();
    ...
    void credit_changes_after_iceberg_order_execution() {
        ...
        orders.forEach(order -> security.getOrderBook().enqueue(order));
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 3, java.time.LocalDateTime.now(), Side.BUY, 445, 15450, 0, 0, 150);
        matcher = new Matcher();
    ...
    void credit_changes_after_order_update_with_different_price() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 6, java.time.LocalDateTime.now(), Side.SELL, 350, 15750, 0, 0, 0);
        matcher = new Matcher();
        matcher.match(orders.get(5).snapshotWithQuantity(350));
        try {
            security.updateOrder(updateOrderRq, matcher);
    ...
    void credit_changes_after_order_update_with_different_quantity() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 6, java.time.LocalDateTime.now(), Side.SELL, 300, 15700, 0, 0, 0);
        matcher = new Matcher();
